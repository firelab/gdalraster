...
Conda is an open-source, cross-platform, language-agnostic package manager and environment management system. The conda package format is general purpose and covers a wide selection of programming languages, including C, C++, Fortran, Rust, Go, Python, and R. Conda uses the concept of channels as the base for hosting and managing packages.

## The origins of conda

The conda packaging format and its associated tooling were initially created by Anaconda (back then Continuum Analytics), as part of their Anaconda Distribution and Miniconda products. The tools were open-source early on, and a platform for free channel hosting was also made available: [anaconda.org](https://anaconda.org/). Soon enough, community-driven initiatives started to use these tools and services to build their own distributions.

These days, the most known community-driven distribution is [conda-forge](https://conda-forge.org/), which is still served from anaconda.org, but also from other mirrors. The original tools written in Python like conda and conda-build are now maintained by the community, and other implementations exist, like Mamba (C++) or Pixi (Rust).

## Conda for R spatial

Conda offers features potentially of interest to R users who work with geospatial data:

* install different versions of binary packages with dependencies on multiple platforms
...
* access R and many CRAN packages, along with current GDAL and its dependencies
...
* plugins are available for format drivers that are sometimes unavailable in other package managers, such as the Arrow and Parquet vector drivers
...
R package names in conda-forge always have the `r-` prefix. When using R in `conda`, packages should be installed with the `conda` package manager rather than from source or binaries using `install.packages()` (i.e., should not mix packages from CRAN or other repositories such as R-universe). Web portals for exploring packages available in conda-forge include https://conda-forge.org/packages/, and the [conda metadata browser](http://conda-metadata-app.streamlit.app/) for more detailed information on each package.
...
The last command starts an interactive subshell with the environment activated (use `exit` to leave the shell). You get a digest-level `pixi.lock` lock file automatically, so the environment is fully reproducible as long as the lock file is committed with the rest of the project.

Pixi also supports using tools and libraries [globally](https://pixi.prefix.dev/latest/global_tools/introduction/), similar to conda's base environment without having to use an activate command:
