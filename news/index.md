...
- [`gdal_usage()`](https://firelab.github.io/gdalraster/reference/gdal_cli.md):
  replace `[may be repeated]` with whether packed values / repeated arg
  allowed or not
  ([\#953](https://github.com/firelab/gdalraster/issues/953))

#### Raster

- class method `GDALRaster$addBand()`: accept an R vector to add a MEM
  band from data pointer without copying
...
- add
  ...
  convenience functions for transforming aspect degrees
...
- redirect `/vsistdout/` to the compliant output stream to allow using
  ...
  ([\#963](https://github.com/firelab/gdalraster/issues/963))
- add
...
- add
  ...
  terminate/reset any active **cli** progress bars
  ([\#939](https://github.com/firelab/gdalraster/issues/939))

### Fixes

- use the `CLI_SHOULD_TICK` macro in the C++ progress bar to avoid
  unnecessary updates on screen for small performance improvement in
  certain cases
  ([\#939](https://github.com/firelab/gdalraster/issues/939))
...
- add new examples for GDAL CLI pipelines and
  ...
  R](https://firelab.github.io/gdalraster/articles/use-gdal-cli-from-r.html#pipeline-examples)
- update (primarily documentation) for the `"UInt8"` pixel data type in
  GDAL \>= 3.13
  ([\#933](https://github.com/firelab/gdalraster/issues/933))
- add links to the new GDAL documentation on [geometry validity and
...
- [`g_make_valid()`](https://firelab.github.io/gdalraster/reference/g_util.md):
  condition tests for changes to `MakeValid()` in GDAL 3.13
...
- add undocumented `.rasterize_polygon()` for internal use, currently
  only for **FIESTA** support
  ([\#955](https://github.com/firelab/gdalraster/issues/955))
- `has_space_char_()`: use `std::any_of` to avoid `cppcheck` style
  warning
...
- add helper functions for working with GDAL raster data types (see
  [`?data_type_helpers`](https://firelab.github.io/gdalraster/reference/data_type_helpers.md))
...
- new additional geometry functions operating on WKT (GEOS via GDAL
  ...
  [`g_crosses()`](https://firelab.github.io/gdalraster/reference/g_binary_pred.md),[`g_overlaps()`](https://firelab.github.io/gdalraster/reference/g_binary_pred.md),
